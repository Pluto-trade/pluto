...
const MarketIcon = ({ symbol }: { symbol: string }) => {
	if (symbol.includes('BTC')) {
		return (
			<div className="flex h-6.5 w-6.5 items-center justify-center rounded-full bg-[#f7931a]">
	...
	if (symbol.includes('ETH')) {
		return (
			<div className="flex h-6.5 w-6.5 items-center justify-center rounded-full bg-[#627eea]">
	...
	if (symbol.includes('SOL')) {
		return (
			<div className="flex h-6.5 w-6.5 items-center justify-center rounded-full bg-black border border-slate-700">
	...
	if (symbol.includes('MATIC')) {
		return (
			<div className="flex h-6.5 w-6.5 items-center justify-center rounded-full bg-[#8247e5]">
	...
	if (symbol.includes('ADA')) {
		return (
			<div className="flex h-6.5 w-6.5 items-center justify-center rounded-full bg-[#ff6b22]">
				<svg width="14" height="14" viewBox="0 0 24 24" fill="white">
	...
	return (
		<div className="flex h-6.5 w-6.5 items-center justify-center rounded-full bg-slate-800">
			...
			<div className="mt-5 flex items-center gap-5 border-b border-[#2a2e39]">
				{tabs.map((tab) => (
					<button
						...
						className={`pb-2.5 text-[11px] font-semibold tracking-wide transition relative ${
							activeTab === tab ? 'text-white' : 'text-[#8e98a8] hover:text-slate-300'
						}`}
					>
						{tab}
						{activeTab === tab && (
							<span className="absolute -bottom-px left-0 w-full h-0.5 bg-indigo-500 rounded-t-full" />
						)}
					</button>
				))}
			</div>

			<div className="mt-3 flex flex-col gap-1 max-h-40 overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-700 [&::-webkit-scrollbar-thumb]:rounded-full">
