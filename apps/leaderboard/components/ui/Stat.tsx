...
export function Stat({
  ...
  return (
    <Card className={cn("relative overflow-hidden", className)}>
      <div className={cn("absolute inset-x-0 top-0 h-px bg-gradient-to-r", accent[tone])} />
      <div className="px-5 py-5">
        <div className="flex items-center justify-between gap-3">
          <span className="text-xs font-medium uppercase tracking-wider text-[var(--color-fg-muted)]">
            {label}
          </span>
          {icon ? <span className="text-[var(--color-fg-dim)]">{icon}</span> : null}
        </div>
        <div className={cn("mt-3 font-mono text-3xl font-semibold tabular-nums", valueColor[tone])}>
          {value}
        </div>
        {hint ? <div className="mt-2 text-xs text-[var(--color-fg-dim)]">{hint}</div> : null}
      </div>
    </Card>
  );
}
